...
export default (ctx: ctx) => {
  ...
  const SCL90 = {
    init: (uid: number) => {
      ctx.logger.info(uid, '初始化了测试')
      session[uid] = {
        index: 0,
        logs: []
      }

      ctx.OneBot.sendPrivateMessage(uid, [
    ...
    result: (uid: number) => {
      ...
      for (const type in s) {
        ...
        msg.push(`${type} (${result}): ${Math.round(score * 1e3) / 1e3}`)
      }

      ctx.logger.info(uid, '结束了测试')
    ...
    next: (uid: number) => {
      ...
      choice.once(String(uid), (c: string) => {
      ...
      ctx.OneBot.sendPrivateMessage(uid, q)
    }
  }

  ctx.OneBot.event.on('private_message', e => {
    const c = ['A', 'B', 'C', 'D', 'E']
    if (c.includes(e.raw_message.trim().toUpperCase())) {
      choice.emit(String(e.sender.user_id), e.raw_message.trim().toUpperCase())
    }
  })

  ctx.command(/^SCL90$/, 'SCL90', '进行一次SCL90自评量表测试（仅限私聊使用）', (m, source, msg, reply) => {
    if (source === 'telegram') return reply('本测试仅限QQ')
    if (msg.group_id) return reply('[SCL90] 不支持群聊中使用此功能')
    SCL90.init(msg.sender.user_id)
  })

  ctx.command(/^开始$/, '开始', '开始SCL90自评量表测试（仅限私聊使用）', (m, source, msg, reply) => {
    if (source === 'telegram') return reply('本测试仅限QQ')
    if (msg.group_id) return reply('[SCL90] 不支持群聊中使用此功能')
    SCL90.init(msg.sender.user_id)
  })
