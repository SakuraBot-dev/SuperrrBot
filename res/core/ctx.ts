...
export default {
  ...
  command: (regexp: RegExp, cmd: string, helper: string, callback: (match: RegExpExecArray | null, source: 'telegram' | 'OneBot', message: any, reply: (msg: any, options?: boolean | any) => void) => void) => {
    ...
    if (bot.Bots.telegram) {
      ...
      tg.event.on('channel_post', ctx => {
        const post = ctx.update.channel_post
        // @ts-ignore
        const reply = (msg: string, options?: any) => {
          ctx.telegram.sendMessage(post.chat.id, msg, options)
        }

        // @ts-ignore
        const m: string = post.text
        regexp.lastIndex = 0
        if (regexp.test(m)) {
          regexp.lastIndex = 0
          logger('Command').info(post.chat.id, ' 在 Telegram 触发了 ', cmd, ' 命令: ', m)
          callback(regexp.exec(m), 'telegram', post, reply)
        }
      })
      ...
      tg.event.on('text', (ctx) => {
        ...
        if (regexp.test(m)) {
          regexp.lastIndex = 0
          logger('Command').info(ctx.message.chat.id, ' 在 Telegram 触发了 ', cmd, ' 命令: ', m)
    ...
    if (bot.Bots.OneBot) {
      ...
      OneBot.event.on('group_message', msg => {
        ...
        regexp.lastIndex = 0
        if (regexp.test(m)) {
          logger('Command').info(msg.sender.nickname, ' 在 OneBot群聊 触发了 ', cmd, ' 命令: ', m)
      ...
      OneBot.event.on('private_message', msg => {
        ...
        regexp.lastIndex = 0
        if (regexp.test(m)) {
          logger('Command').info(msg.sender.nickname, ' 在 OneBot私聊 触发了 ', cmd, ' 命令: ', m)
          callback(regexp.exec(m), 'OneBot', msg, reply)
        }
      })
